...
class Encoder(nn.Module):
    ...
    def load_pretrained_vectors(self, opt, logger):
        ...
        if opt.pre_word_vecs_enc is not None and not opt.disable_pretrained_emb:
...
class Pointer(nn.Module):
    def __init__(self, opt, dicts=None):
        self.opt = opt
        self.layers = opt.layers
    ...
    def forward(self, hidden, context, doc_sent_mask, src_pad_masks, pre_att_hard,
        ...
        :param max_step: (the max step is set to 1 in Summarizer.py)
        ...
        for i in range(max_step):
            ...
            if self.training and max_step > 1:
                # Max step is set to 1 in Summarizer.py, so this might be skipped
                max_idx = prev_targets[i]
                if self.use_gpu:
                    hard_max_idx = get_hard_attention_index(context.size(0), context.size(1), max_idx).cuda()
                else:
                    hard_max_idx = get_hard_attention_index(context.size(0), context.size(1), max_idx)
...
class DecInit(nn.Module):
    ...
    def forward(self, last_enc_h):
        ...
        return torch.tanh(self.initer(last_enc_h))
...
class NMTModel(nn.Module):
    def __init__(self, sent_encoder: Encoder, doc_encoder: DocumentEncoder, pointer: Pointer, decIniter, reward_cal=None):
    ...
    def gen_all_masks(self, base_mask, targets):
        ...
        for i in range(targets.size(1)):
            ...
            next_mask = Variable(next_mask, requires_grad=False)
    ...
    def forward(self, input):
        ...
        doc_hidden, doc_context, doc_sent_mask = self.encode_document(input[0], input[3])
        ...
        # ipdb.set_trace()
        # max_point_step = input[2][1].max().data[0]
        max_point_step = input[2][1].max().item()
        prev_att = init_att
        pointer_precompute_hard = None

        all_masks = self.gen_all_masks(doc_sent_mask, input[2][0])

        oracle_targets = input[2][0]
